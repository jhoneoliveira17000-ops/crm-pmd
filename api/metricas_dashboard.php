...
try {
    ...
    // 11. Total Leads
    $stmt = $pdo->query("SELECT COUNT(*) FROM leads WHERE " . get_tenant_condition());
    ...
    $stmt = $pdo->query("SELECT SUM(valor_estimado) FROM leads WHERE status_id != 4 AND (" . get_tenant_condition() . ")");
    $pipeline_value = $stmt->fetchColumn() ?? 0;

    // 13. Conversão (Leads Fechados / Total de Leads)
    $stmt = $pdo->query("SELECT COUNT(*) FROM leads WHERE status_id = 4 AND (" . get_tenant_condition() . ")"); // 4 = Fechado
    ...
    // 15. Leads by Source
    $stmt = $pdo->query("SELECT origem, COUNT(*) as count FROM leads WHERE " . get_tenant_condition() . " GROUP BY origem ORDER BY count DESC");
